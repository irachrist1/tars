...
Only if no workspace exists anywhere is this session onboarding: a job interview for you, not a setup wizard. The user is not technical. **Read `references/onboarding.md` and follow it.** The shape: detect what you have to work with (connector, file access, where OneDrive is, which surfaces they use) → one question ("where is your work kept?") → go read it → come back uncannily specific and settle anything you couldn't place → four more plain questions (how they write, what matters, boundaries) → show the proposed workspace and write it only on a yes → prove yourself immediately on one real question or a meeting prep, with a citation → **publish the skill to every surface they named** (`node scripts/package.mjs`, then upload via Customize → Skills — see `PUBLISHING.md`). Setup is not complete until the skill works everywhere they work, not just on this machine. Under 15 minutes, value in the first 5, no ceremony at the end.
