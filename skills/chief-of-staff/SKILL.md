...
- **Build the local index (the fast lane for "find/pull X"):** `tars index build --root "<their work folder>"` (or `node scripts/indexer.mjs build …`) — persistent BM25 index in `<work folder>/.tars-index`. Query: `tars index query "<words>" --json`. `tars open` runs incremental `update` when an index exists. See `references/indexer.md`.
- **Unified context (meeting prep):** `tars context "<question>" --root "<folder>" --json` merges file hits, connector routes, and client brief pointers (`scripts/context-engine.mjs`).
- **Precedents:** `node scripts/precedents.mjs --root "<folder>" --client ACME --type proposal --json`
- **Connector map (once per session, cheap):** `node scripts/connectors.mjs` — when `claude mcp list` isn't available, pass visible `mcp__*` tools via `--tools`. `tars doctor` gives a one-screen health check. Keep `CONNECTORS.md` current from the connector map.
...
Only if no workspace exists anywhere is this session onboarding: a job interview for you, not a setup wizard. The user is not technical. **Read `references/onboarding.md` and follow it.** The shape: detect what you have to work with (connector, file access, where OneDrive is) → one question ("where is your work kept?") → go read it → come back uncannily specific and settle anything you couldn't place → four more plain questions (how they write, what matters, boundaries) → show the proposed workspace and write it only on a yes → prove yourself immediately on one real question or a meeting prep, with a citation → publish to every surface they use → offer handoff (`references/handoff.md`: launch Claude with the prompt or give them a copyable block). Under 15 minutes, value in the first 5, no ceremony at the end.
